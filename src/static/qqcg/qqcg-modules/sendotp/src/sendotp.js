define(function(require, exports, module) {
	...
	var SendOTP = function(options) {
		this.otps = $.extend({}, SendOTP.defaults, options);
		this.init();
	}
	...
	SendOTP.prototype = {
		...
		handleSend: function(){
			...
			otps.$otpBtn.bind('click',function(){
		        ...
		        otps.$otpBtn.data('sendOTPCount',61).data('sendOTPtimeout', setInterval(countdownFn, 1000));
		        countdownFn();
		        //发送验证码请求逻辑
		        self.getOtp();
		    });
		...
		changeSendData: function(data) {
			var otps = this.otps;
			if(data){
				otps.sendData = data;//data showed be json
			}
		}
	};
	SendOTP.defaults = {
		$otpBtn : $('#otpSend'), //current otp button
		$otpValue : $('#otpValue'), //verifycode
		//phoneNumber: '', //mobile number
		btnText : "免费获取",
		sendData : {}
	};

    SendOTP.create = SendOTP.prototype.create;

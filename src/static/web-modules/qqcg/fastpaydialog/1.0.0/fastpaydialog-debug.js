 ...
 */
define("consumer/fastpaydialog/1.0.0/fastpaydialog-debug", [ "consumer/dialog/1.0.0/dialog-debug" ], function(require, exports, modules) {
    var doc = document, ArrayProto = Array.prototype;
    var Dialog = require("consumer/dialog/1.0.0/dialog-debug"), DialogProto = Dialog.prototype;
    var FastPayDialog = function(opts) {
        var options = this.options = extend(FastPayDialogProto.defaultOptions, opts);
        this.init(options);
    };
    var readyState = true, ERROR_TMPL = '<div class="box-error">系统异常，请稍后再试</div>';
    ...
    var FastPayDialogProto = FastPayDialog.prototype = extend(DialogProto, {
        ...
        createDialogContent: function(options) {
            var dialog = this.dialog, dialogInstance = this;
            _getListData(options.url, function(ajax) {
                var containD = dialogInstance.options.payCardTypes.indexOf("D") != -1, containC = dialogInstance.options.payCardTypes.indexOf("C") != -1, dialogHmtl = '                        <ul class="tabs-nav">' + (containD ? "<li>储蓄卡</li>" : "") + (containC ? "<li>信用卡</li>" : "") + "</ul>", IS_HIDE = true, data = eval("(" + ajax.responseText + ")"), firstPayCard;
                try {
                    firstPayCard = data.debitList[0];
                    ...
                    dialogHmtl += '                            <div class="btns">                                <button id="bfpDialogConfirmBtn" class="btn btn-primary btn-confirm">确定</button>                            </div>';
                } catch (e) {
                    readyState = false;
        ...
        removeDialog: function() {
            ...
            try {
                var dialog = this.dialog, lists = dialog.getElementsByTagName("ul"), tabNavs = lists[0].getElementsByTagName("li"), bankCodeInputs = dialog.getElementsByTagName("input"), confirmBtn = doc.getElementById("bfpDialogConfirmBtn");
        ...
        bandFastPayDialogEvents: function() {
            var dialogInstance = this, dialog = this.dialog, lists = dialog.getElementsByTagName("ul"), tabNavs = lists[0].getElementsByTagName("li"), tabs = function() {
                var uls = slice(lists, 1), tabs = [];
                for (var index in uls) {
                    tabs.push(uls[index].parentNode);
                }
                return tabs;
            }(), bankCodeInputs = dialog.getElementsByTagName("input"), payCardTypes = [ "D", "C" ], preBankCodeInput = bankCodeInputs[0], confirmBtn = doc.getElementById("bfpDialogConfirmBtn");
            for (var i = 0, len = tabNavs.length; i < len; i++) {
        ...
        displayDilogContent: function(htmlTmpl) {
            var dialog = this.dialog, loadingBox = doc.getElementById("loadingBox"), dialogContent = doc.createElement("div");
            dialogContent.innerHTML = htmlTmpl;
            loadingBox.style.display = "none";
            dialog.insertBefore(dialogContent, loadingBox);
        },
        showMoreBankCards: function() {
            var dialog = this.dialog, moreDBankCards = doc.getElementById("moreInetBankCards_D"), moreCBankCards = doc.getElementById("moreInetBankCards_C");
            if (moreDBankCards) {
    ...
    * */
    ...
    function _concatBankListTmpl(list, isHide, cardType) {
        var listTmpl = '<div class="tab"' + (isHide ? ' style="display:none;"' : "") + '>                                <p class="line">选择要添加的银行卡</p>                                <ul id="more_' + cardType + '" class="banks">{{liTmpl}}</ul>                            </div>', liTmpl = "", bank;
        for (var i = 0, len = list.length; i < len; i++) {
            bank = list[i];
            liTmpl += '<li class="' + (i > 19 ? "hidden" : "") + " bank " + (!isHide && i == 0 ? "active" : "") + '">                                <label><input name="bankCode" type="radio"' + (!isHide && i == 0 ? "checked" : "") + ' data-bankid="' + bank.bankId + '"  value="' + bank.bankCode + '" />&nbsp;&nbsp;                                    <span class="box-icon"><i class="icon-bank icon-bank-' + (bank.bankCode.toLowerCase() || "pab") + '" >' + bank.bankName + "</i></span></label>                            </li>";
            if (i == len - 1 && len > 20) {

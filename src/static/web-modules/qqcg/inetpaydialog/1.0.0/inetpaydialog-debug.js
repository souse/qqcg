 ...
 */
define("consumer/inetpaydialog/1.0.0/inetpaydialog-debug", [ "consumer/dialog/1.0.0/dialog-debug" ], function(require, exports, modules) {
    ...
    var InetPayDialogProto = InetPayDialog.prototype = extend(DialogProto, {
        ...
        createDialogContent: function(options) {
            ...
            _getListData(options.url, function(ajax) {
                var loadingBox = doc.getElementById("loadingBox"), dialogHmtl = '<h3><span class="title">选择银行：</span>需跳转银行页面，不享受平安保险保障</h3>', list = eval("(" + ajax.responseText + ")"), firstPayCard;
                try {
                    firstPayCard = list[0];
                    ...
                    dialogHmtl += '                            <div class="btns">                                <button id="inetBankDialogConfirmBtn" class="btn btn-primary btn-confirm">确定</button>                            </div>';
                } catch (e) {
                    readyState = false;
        ...
        removeDialog: function() {
            ...
            try {
                var dialog = this.dialog, bankCodeInputs = dialog.getElementsByTagName("input"), confirmBtn = doc.getElementById("inetBankDialogConfirmBtn");
        ...
        bandInetPayDialogEvents: function() {
            var dialogInstance = this, dialog = this.dialog, bankCodeInputs = dialog.getElementsByTagName("input"), preBankCodeInput = bankCodeInputs[0], confirmBtn = doc.getElementById("inetBankDialogConfirmBtn");
            for (var i = 0, len = bankCodeInputs.length; i < len; i++) {
    ...
    function _concatBankListTmpl(list) {
        var listTmpl = '<div class="tab">                                <ul id="inetBanks" class="banks">{{liTmpl}}</ul>                            </div>', liTmpl = "", bank;
        for (var i = 0, len = list.length; i < len; i++) {
            bank = list[i];
            liTmpl += '<li class="' + (i > 19 ? "hidden" : "") + " bank " + (i == 0 ? "active" : "") + '">                                <label><input name="bankCode" data-paycardtype="' + bank.dcCardType + '" data-bankcode="' + bank.bankCode + '" data-bankName="' + bank.bankName + '" type="radio" ' + (i == 0 ? "checked" : "") + ' value="' + bank.bankId + '" />&nbsp;&nbsp;                                <span class="box-icon"><i class="icon-bank icon-bank-' + (bank.bankCode.toLowerCase() || "pab") + '" >' + bank.bankName + "</i></span></label>                            </li>";
            if (i == len - 1 && len > 20) {

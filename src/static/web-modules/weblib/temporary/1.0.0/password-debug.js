(function() {
            ...
            (function($) {
                ...
                $.fn.passFormSub = function() {
                    ...
                    $(this).find(".encrypted-password, .plugin-encrypted-password").each(function() {
                        var $t = $(this), holder = $("#" + $t.prop("id") + "_");
                        //holder.val(cryptk($t.val(), data.ts, data.hPK, data.aPK));
                        var $tv = $t.val().trim();
                        if ($tv && $tv.length > 0) {
                            holder.val(cryptk($tv, data.ts, data.hPK, data.aPK));
                        }
                    });
                };
                $.fn.passwordify = function() {
                    $(this).each(function() {
                        var $t = $(this);
                        $("<input type='hidden' name='" + $t.prop("name") + "' id='" + $t.prop("id") + "_" + "'/>").appendTo($t.closest("form"));
                        var el = $t.css("visibility", "visible")[0];
                        el.removeAttribute("disabled");
                        el.removeAttribute("name");
                    });
                };
                $(".encrypted-password, .plugin-encrypted-password").passwordify();
            })(jQuery);
        } else {
            setTimeout(run, 50);
        }
    ...
    NS("PINGANFU.util.password", function() {
        //rCharsCount[0] rCharsCount[1]  rCharsCount[2]  rCharsCount[3] 为合法字符，非这些字符，则为不合法字符
        var rCharsCount = [ [ /[A-Z]/, 26 ], [ /[a-z]/, 26 ], [ /[0-9]/, 10 ], [ /[~!\@\#\$\%\*\^\&\(\)\;.?\<\>\\_\/\-`]/, 21 ] ], rValid = /[^A-Za-z0-9~!\@\#\$\%\*\^\&\(\)\;\<\>.?\\_\/\-`]/, maxEntropy = Math.log(83) / Math.LN2, midLength = 11;
        this.entropyScore = function(v) {
            var i = 0, n = 0;
        ...
        this.validCharSet = function(v) {
            return !rValid.test(v);
        };
        // 必须两种组合以上
        this.groupScore = function(v) {
            var i = 0, n = 0;
            for (;i < rCharsCount.length; i++) {
                if (rCharsCount[i][0].test(v)) {
                    n += 1;
        ...
        this.validCharSetGroup = function(v) {
            return !rValid.test(v) && this.groupScore(v) >= 2;
        };
        this.lessLength = function(v, num) {
            var value = v || "", num = num || 6;
            return value.length < num;
        };
        this.moreLength = function(v) {
            var value = v || "";
            return value.length > 16;
        };
    });
    (function($) {
        $(".password-ruled").on("validate", function(e, vc) {
            var pUtils = PINGANFU.util.password, $password = $(this), val = $password.val();
            if (!pUtils.validCharSet(val)) {
                $(this).trigger("displayStrength", [ -1 ]);
                vc.reject("密码不可用");
                return false;
            }
            if ($password.hasClass("password-login") && pUtils.lessLength(val, 8)) {
                $(this).trigger("displayStrength", [ -1 ]);
                vc.reject("密码长度需要是8位以上");
                return false;
            } else if (pUtils.lessLength(val)) {
                $(this).trigger("displayStrength", [ -1 ]);
                vc.reject("密码长度需要是6位以上");
                return false;
            }
            if (pUtils.moreLength(val)) {
                $(this).trigger("displayStrength", [ -1 ]);
                vc.reject("密码长度需要是16位以下");
                return false;
            }
            // 只针对登陆密码
            if ($password.hasClass("password-login") && !pUtils.validCharSetGroup(val)) {
                $(this).trigger("displayStrength", [ -1 ]);
                vc.reject("大写字母、小写字母、数字、特殊字符至少包含2种");
                return false;
            }
        });
        $(".password-ruled").on("afterValidate", function(e, vc) {
            var pUtils = PINGANFU.util.password, val = $(this).val();
            if (val == "" || !pUtils.validCharSet(val)) {
                $(this).trigger("displayStrength", [ -1 ]);
                return false;
            }
            var score = pUtils.entropyScore(val), mark = 0;
            if (pUtils.testGreyList(val)) {
        ...
        $("body").on("displayStrength", ".passwordHolder", function(e, mark, score, greyList) {
            var str = "invalid super-weak weak medium strong".split(" ");
            for (var i = 0; i < str.length; i++) {
                $(this).removeClass(str[i]);
            }
            $(this).addClass(str[mark + 1]);
            $(this).find("input.strength").val(mark);
            if (greyList) {
                if ($(this).next(".help-block").length > 0) {
                    $(this).next(".help-block").html("灰名单");
                } else {
                    $(this).children(".greyListTip").html("灰名单");
                    $(this).children(".greyListTip").show();
                }
            } else {
    ...
    if (typeof define === "function") {
        define("pafweblib/temporary/1.0.0/password-debug", [], function() {});
    }
})();
